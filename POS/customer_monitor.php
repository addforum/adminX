...
<div class="container">
    <table class="table table-bordered">
        ...
        <?php
        $get_cust="Select * from cust_master where cust_status='ON' AND cust_last_apoint_outlet_id='$pos_id' ORDER BY cust_last_apoint_date DESC ";
        $run_cust=mysqli_query($con, $get_cust);
        while($row_cust=mysqli_fetch_array($run_cust)){
        
        $cust_name=$row_cust['cust_name'];
        $cust_mobile=$row_cust['cust_mobile'];
        $last_appoint=$row_cust['cust_last_apoint_date'];
        
        ?>
        <tr>
            <td><?php echo $cust_name; ?></td>
            <td><?php echo $cust_mobile; ?></td>
            <td><?php echo $last_appoint; ?></td>
        </tr>
        <?php } ?>
    </table>
</div>
